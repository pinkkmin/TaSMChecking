...
private:
  ...
  Function *m_f_printfBaseBoundDebug;
...
public:
  ...
  void insertCallSiteDebugPrintPtrfFunc(Value *ptr, Value *type,
  ...
  void insertCallSiteDebugPtrBaseBound(SmallVector<Value *, 8> args,
                                       Instruction *insert_at) {

    CallInst::Create(m_f_printfBaseBoundDebug, args, "", insert_at);
  }
};
} // namespace
...
void tasmChecking::initTypeName(Module &module) {
  ...
  m_zero = ConstantInt::get(Type::getInt32Ty(module.getContext()), 0, false);
  m_one = ConstantInt::get(Type::getInt32Ty(module.getContext()), 1, false);
}
...
void tasmChecking::constructOthersHandlers(Module &module) {
  ...
  module.getOrInsertFunction("_f_printfPointerDebug", VoidTy, VoidPtrTy,
                             SizeTy);
  module.getOrInsertFunction("_f_printfPtrBaseBound", VoidTy, VoidPtrTy,
                             VoidPtrTy, VoidPtrTy);
...
void tasmChecking::getOthersFunctions(Module &module) {
  ...
  m_f_printfBaseBoundDebug = module.getFunction("_f_printfPtrBaseBound");
  assert(m_f_printfBaseBoundDebug && "m_f_printfBaseBoundDebug is NULL ? ");
}
/**********************************************************************************************************************************/
...
void tasmChecking::getConstantExprBaseBound(Constant *given_constant,
  ...
  if (cexpr) {
    ...
    Constant *gep_bound =
        ConstantExpr::getGetElementPtr(nullptr, given_constant, indices_bound);
...
void tasmChecking::insertLoadStoreChecks(Instruction *Inst) {
  ...
  if (isa<LoadInst>(Inst)) {
    ...
    // insertCallSiteDebugPrintPtrfFunc(pointer_operand, Inst);
  }
  ...
  if (isa<LoadInst>(Inst)) {
    ...
    // Value *maskedPtr = insertCallSiteMaskPtr(pointer, Inst);
    ...
    // StoreInst *sti = dyn_cast<StoreInst>(Inst);
    // Value *pointer = sti->getOperand(1);
    // Value *maskedPtr = insertCallSiteMaskPtr(pointer, Inst);
    // sti->setOperand(1, maskedPtr);
...
Value *tasmChecking::insertCallSiteMaskPtr(Value *ptr, Instruction *insert_at) {
  ...
  SmallVector<Value *, 8> args;
  args.push_back(ptr);
  Value *maskedPtr = CallInst::Create(m_f_maskingPointer, args, "", insert_at);
  return maskedPtr;
}
...
void tasmChecking::getStackPtrTypeKey(Instruction *inst) {
  ...
  // Value *functionKey = m_func_key_pool[funcId];
  // insertCallSiteDebugPrintPtrfFunc(funcKey, m_one, inst);
  // insertCallSiteDebugPrintPtrfFunc(funcId, m_zero, inst);
}
